...
#if CONFIG_IDF_TARGET_ESP32S31 && CONFIG_LK_CUSTOM_S31_BOARD_FUNCTION_CORE
...
static esp_err_t init_i2s(void)
{
    ...
    // Match the component adapter: retain 240-frame DMA blocks and allow
    // 120 ms of queue capacity for stalls while AEC processes capture.
...
static esp_err_t create_codec_devices(void)
{
    ...
    int ret = esp_codec_dev_set_in_gain(
        s_record_device, (float)CONFIG_LK_CUSTOM_S31_FUNCTION_CORE_MIC_GAIN_DB);
...
esp_err_t s31_function_core_audio_init(void)
{
    ...
    ESP_LOGI(TAG,
             ...
             FUNCTION_CORE_PA_CTRL,
             CONFIG_LK_CUSTOM_S31_FUNCTION_CORE_MIC_GAIN_DB);
    return ESP_OK;
}
...
#endif // ESP32-S31 Function Core profile
